...
\begin{lstlisting}[caption={[Einfacher Webserver] Einfacher Webserver \protect \footnotemark},label=lst:Middleware]
...
\noindent
\textbf{Middleware:}
Express arbeitet nach dem Middleware-Konzept. Darunter versteht man Funktionen, die für die Verarbeitung von Anfragen hintereinandergeschaltet werden können. Jede Middleware hat Zugriff auf das Anfrageobjekt, das Antwortobjekt und die jeweils nächste Middleware-Funktion (\cite{Node1.9}, S. 895ff).
...
\caption[Middleware]{Middleware (\cite{Node1.2}, S. 868ff)}
...
\begin{lstlisting}[caption={[Routinghandler erstellen] Routinghandler erstellen \protect \footnotemark},label=lst:RoutingHandlerCreate]
...
\begin{lstlisting}[caption={[Mongoose Verbindungsoptionen] Mongoose Verbindungsoptionen \protect \footnotemark},label=lst:MongooseConnect]
const options = {
