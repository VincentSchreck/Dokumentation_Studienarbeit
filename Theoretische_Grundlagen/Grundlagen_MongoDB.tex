...
\hangindent1cm
\textbf{Delete:}
Für das Löschen von Datensätzen aus einer Collection gibt es die \texttt{DeleteOne()} und \texttt{DeleteMany()} Methoden. Über mitgegegeben Parameter können Filter eingestellt werden. Eine ganze Collection kann mit der \texttt{drop()}  Methode entfernt werden.
\newline
...
Der \enquote{mongod}-Prozess leitet Anfragen an die jeweiligen Shards weiter und dient als Schnittstelle zwischen den Clientanwendungen.
